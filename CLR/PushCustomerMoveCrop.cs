...
public class PushCustomerMoveCrop
{
        ...
        String station, String router, out SqlString text)
    {
        ...
        try
        {
            ...
            {
                //code is code of customer
                // phone is phone of customer
                // name is name of customer
                // invoice is code of invoice
                // station is address of customer or station
                // router is code of router api
            ...
            {
                responSQL = (httpResponse.StatusCode == HttpStatusCode.Created) ? "OK" : "ERROR";
                httpResponse.Close();
            }
        }
        catch (Exception e)
        ...
        String station, String router)
    {
        ...
        return "{\"invoice_id\":\"" + invoice.Trim() + "\"," +
               ...
               "\"cod\":\"\"," +
               "\"order_weight\":\"\"," +
               "\"order_note\":\"" + invoice.Trim() + " - " + code.Trim() + " - " + name.Trim() + "\"," +
               "\"price_incurred\":\"\"," +
               "\"order_length\":\"\"," +
               "\"order_width\":\"\"," +
               "\"order_height\":\"\"," +
               ...
               "\"order_shipping_address\":\"" + name.Trim() + " - " + station.Trim() + "\"," +
    ...
    public static void CreateRouter(String email, String code, out SqlString text)
    {
        ...
        try
        {
            using (var streamWriter = new StreamWriter(httpWebRequest.GetRequestStream()))
            {
                string json = GetBodyRouter(MyDic3[email.Trim()], code.Trim());
                streamWriter.Write(json);
                streamWriter.Close();
            }
            ...
            if (response.StatusCode == HttpStatusCode.Created)
            {
                using (StreamReader streamReader = new StreamReader(response.GetResponseStream()))
                {
                    ...
                    responSQL = res.Substring(from, to - from);
                    streamReader.Close();
                }
            }
            else
    ...
    private static string GetBodyRouter(long shipper_id, String code)
    {
        // get today
        DateTime today = DateTime.Today;
